...
if __name__ == "__main__":
    ...
    for idx_folder, specific_folder_name in enumerate(selected_list):
        ...
        if  to_infere_LR_results_label:
            ...
            clf = LogisticRegression(C=logreg_c, random_state = 47, class_weight='balanced')
            clf.fit(nfeatures_tr, labels_tr)
            
            # Save model
            ckpt_dir = os.path.join(results_output_path, 'optuna_tuning', 'objective_LR_MCI', '1d_365d')
        ...
        if to_calculate_bootstrapped_performance:
            ...
            # Load saved model
            model_path = os.path.join(results_output_path, 'optuna_tuning', 'objective_LR_MCI', '1d_365d', 'test_model.p')
            ...
            for data, name in [(precision, 'precision'), (recall, 'recall')]:
                with open(os.path.join(results_output_path, f'{name}_LR_1d_365d.pkl'), 'wb') as f:
            ...
            df.to_csv("confidence_interval_LR_1d_365d.csv", index=False)
