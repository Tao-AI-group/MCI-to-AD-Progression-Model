...
if __name__ == "__main__":
    ...
    for idx_folder, specific_folder_name in enumerate(selected_list):
        ...
        to_infere_XGBoost_results_label = 1
        if  to_infere_XGBoost_results_label:
            ...
            num_positive = np.sum(labels_tr == 1)
            num_negative = np.sum(labels_tr == 0)
            ...
            # Save model
            ckpt_dir = os.path.join(results_output_path, 'optuna_tuning', 'objective_XGBoost_MCI', '1d_365d')
        ...
        # Bootstrap peXGBoostormance evaluation 
        to_calculate_bootstrapped_peXGBoostormance = 0
        if to_calculate_bootstrapped_peXGBoostormance:
            
            # Load saved model
            model_path = os.path.join(results_output_path, 'optuna_tuning', 'objective_XGBoost_MCI', '1d_365d', 'test_model.p')
            ...
            for data, name in [(precision, 'precision'), (recall, 'recall')]:
                with open(os.path.join(results_output_path, f'{name}_XGBoost_1d_365d.pkl'), 'wb') as f:
                    pickle.dump(data.tolist(), f)
                
            
            # PeXGBoostorm bootstrap analysis
            n_trials = 500
            alpha = 0.05
            ...
            df.to_csv("confidence_interval_XGBoost_1d_365d.csv", index=False)
