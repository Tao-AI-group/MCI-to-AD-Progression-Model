...
if __name__ == "__main__":
    ...
    for idx_folder, specific_folder_name in enumerate(selected_list):
        ...
        to_infere_RF_results_label = 1
        if  to_infere_RF_results_label:
            ...
            # Save model
            ckpt_dir = os.path.join(results_output_path, 'optuna_tuning', 'objective_RF_MCI', '1d_365d')
        ...
        if to_calculate_bootstrapped_performance:
            ...
            # Load saved model
            model_path = os.path.join(results_output_path, 'optuna_tuning', 'objective_RF_MCI', '1d_365d', 'test_model.p')
            ...
            for data, name in [(precision, 'precision'), (recall, 'recall')]:
                with open(os.path.join(results_output_path, f'{name}_RF_1d_365d.pkl'), 'wb') as f:
            ...
            df.to_csv("confidence_interval_RF_1d_365d.csv", index=False)
